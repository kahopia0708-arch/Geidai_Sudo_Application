...
**ユーザー操作（Unity 側）**
- [x] Unity エディタで `com.unity.ai.assistant`（AI Assistant）パッケージを導入（`com.unity.ai.assistant` 2.9.0-pre.2 導入済み）
- [x] Unity AI サブスク有効化・Unity Cloud 連携
- [x] `Edit > Project Settings > AI > Unity MCP Server` で **ブリッジを Start**
- [x] Cursor を relay 経由で接続（`user-unity-mcp` として出現）
- [x] 初回接続時の **Pending Connection を承認**
...
- [x] FsCheck 導入方針の確定 → **NuGetForUnity（UPM git）で FsCheck 2.16.6 + FSharp.Core 4.7.2**

**前提/注意**
- [x] コンパイルエラーが無いこと（ベースライン Error 0 を確認）
- [x] 未接続時フォールバック（§5）は不要（接続済みで検証実施）
- _トレース: US-TECH-05 / NFR-10（変更管理）_

### Step 1: AsmDef とフォルダ基盤
- [x] `Assets/Scripts/Common/Geidai.Common.asmdef`（references: UnityEngine.UI, autoReferenced=true）
- [x] `Assets/Scripts/Services/Geidai.Services.asmdef`（references: Geidai.Common）
- [x] `Assets/Scripts/Tests/EditMode/Geidai.Tests.asmdef`（Editor専用、references: Common, Services；FsCheck/FSharp.Core/nunit、UNITY_INCLUDE_TESTS）
- _トレース: NFR-08（モジュール分割）_

### Step 2: Common — ドメインモデル
- [x] `Common/Models/SceneId.cs`（enum: Boot, Home, Register, Rec, Collection, Theme, Game1；Place は含めない/BR-15）
- [x] `Common/Models/UserProfile.cs`（birthYear[1900..現在], nickname[1..8]；BR-01/02）
- [x] `Common/Models/SoundClipMeta.cs`（id(GUID), displayName, createdAtIso, wavFileName）
- [x] `Common/Models/SoundEffectSettingsData.cs`（pitchSemitones ±12, NoiseLevel(4), TimbreType(3), reverb 0..1／JsonUtility 可能）
- [x] `Common/Models/AudioBuffer.cs`（44100Hz/mono/16bit/3s=132300 samples）
- [x] `Common/Models/SavedSound.cs`（SoundClipMeta＋SoundEffectSettingsData の対）
- _トレース: Functional Design domain-entities.md / NFR-08_

### Step 3: Common — Result 型
- [x] `Common/Results/ResultCode.cs`（enum: Ok, NotFound, Corrupted, IOError, ValidationError, Unknown, NotImplemented）
- [x] `Common/Results/Result.cs`（`Result` / `Result<T>`：IsSuccess, Code, Message, Value）
- _トレース: NFR-07 / nfr-design-patterns §1_

### Step 4: Common — 純粋関数（PBT対象）
- [x] `Common/Audio/WavCodec.cs`（static Encode(float[]→byte[]) / Decode(byte[]→WavData)；16bit PCM）
- [x] `Common/Audio/PitchMath.cs`（static CentsToRatio / RatioToCents / SemitonesToCents / Clamp 等）
- _トレース: NFR-09 / business-logic-model.md_

### Step 5: Common — ユーティリティ
- [x] `Common/Utils/ValidationUtil.cs`（ValidateBirthYear, ValidateNickname → Result；BR-01〜03）
- [x] `Common/Utils/SafeLogger.cs`（PII マスク付きログ、本番は詳細抑制）
- _トレース: Security/NFR-04 / nfr-design-patterns §3_

### Step 6: Common — UI 基盤
- [x] `Common/UI/ResponsiveCanvasConfigurator.cs`（CanvasScaler=ScaleWithScreenSize, 参照1080×1920, Match0.5）
- [x] `Common/UI/SafeAreaFitter.cs`（Screen.safeArea 追従、向き/解像度変更で再適用、差分間引き）
- [x] `Common/UI/UITheme.cs`（ScriptableObject：配色/フォント/アイコン参照；Sさん調整点）
- [x] `Common/UI/ScreenRootBase.cs`（abstract：Show→Configure→ApplySafeArea→固有初期化、OnBackPressed/BackRequested）
- [x] `Common/UI/ErrorPresenter.cs`（バナー＋最小実装、アイコン＋平易文言、警告/Result 表示）
- [x] （MCP）コンパイル確認: `Unity_GetConsoleLogs`（Error 0）＋ WavCodec/PitchMath スモーク成功
- [x] （MCP）`UITheme` 既定アセット作成: `Unity_RunCommand` で `Assets/Settings/UITheme_Default.asset` を生成
- _トレース: US-TECH-01/02/07 / NFR-11/12/05 / frontend-components.md_

### Step 7: Services — 器と登録
- [x] `Services/ServiceRegistry.cs`（軽量サービスロケータ：Register<IF>/Resolve<IF>/IsRegistered/Clear）
- [x] `Services/AppManager.cs`（Bootstrap：サービス登録→profile読込→初回遷移判定；MonoBehaviour エントリ）
- _トレース: NFR-06/07/08 / logical-components.md §2.1_

### Step 8: Services — Navigation
- [x] `Services/Navigation/INavigationService.cs`（GoTo(SceneId), GoBack, Current）
- [x] `Services/Navigation/NavigationService.cs`（SceneId→実シーン名マップ、Place 除外/BR-15；SceneManager ラップ＋履歴）
- _トレース: US-TECH-04 / FR-02 / BR-12〜15_

### Step 9: Services — Storage（U1 最小）
- [x] `Services/Storage/IStorageService.cs`（LoadProfile/SaveProfile/ListSounds/LoadSound → Result）
- [x] `Services/Storage/StorageService.cs`（persistentDataPath、基本保存＋対ファイル整合チェック/スキップ、JsonUtility；原子的置換は U4）
- _トレース: NFR-07 / BR-04〜07 / nfr-design-patterns §1.2_

### Step 10: Services — Audio / Content（器）
- [x] `Services/Audio/IAudioService.cs`（録音/再生 IF；実装は U3。器のみ）
- [x] `Services/Content/IContentService.cs`（コンテンツ取得 IF）
- [x] `Services/Content/ContentService.cs`（器＋ NotImplemented を Result で返す）
- [x] （MCP）コンパイル確認: `Unity_GetConsoleLogs`（Error 0）
- _トレース: application-design services.md / NFR-05_

### Step 11: Tests（生成・PBT）
- [x] NuGetForUnity 導入 → FsCheck 2.16.6 + FSharp.Core 4.7.2（packages.config）
- [x] `Tests/EditMode/WavCodecTests.cs`（FsCheck：Encode→Decode ラウンドトリップ不変・フォーマット一致）
- [x] `Tests/EditMode/PitchMathTests.cs`（FsCheck：Cents↔Ratio 逆変換、semitone→cents 線形）
- [x] `Tests/EditMode/SerializationTests.cs`（JsonUtility：SoundEffectSettingsData / UserProfile ラウンドトリップ）
- [x] （MCP）テストアセンブリのコンパイル確認（Error 0）＋ 不変条件の同期スモーク（各500ケース PASS）。正式 NUnit 実行は Build & Test（TestRunner 非同期は MCP 一発実行では結果取得不可）
- _トレース: NFR-09_

### Step 12: コード生成サマリ（ドキュメント）
- [x] `aidlc-docs/construction/u1-foundation/code/code-summary.md`（生成ファイル一覧、名前空間、TODO、U3/U4 統合予定、Unity 取り込み手順、MCP 検証結果、次ユニットの MCP シーン適用メモ）
- _注: サマリのみ aidlc-docs 配下。コードは Assets 配下。_

### Step 13: ストーリー完了マーク
- [x] US-TECH-01/02/04/07 の U1 該当分（基盤コード）を実装済みマーク、US-TECH-05 は開発規約として運用開始
