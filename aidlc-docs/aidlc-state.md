...
- [x] Infrastructure Design — SKIP（完全オフライン・サーバー/クラウド無し／実行計画と整合）
- [~] Code Generation — Part1 詳細計画（Step0〜13）作成・承認待ち（2026-07-16）。ThemeItem/ThemeCatalog(SO)/ThemeSelector を Geidai.Common、IContentService 後方互換拡張(GetCurrentTheme)＋ContentService 実装＋ThemeContext を Geidai.Services.Content、新 Geidai.Theme に WeeklyThemeController/WeeklyThemeScreenController、EditMode テスト（ThemeSelector PBT/ContentService 単体）、旧 WeeklyTextController は残置（差替後削除＝MCP フォローアップ）。
